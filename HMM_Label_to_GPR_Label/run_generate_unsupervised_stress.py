    ...
    for line in lines:
        ...
        if (match.group('cur_phoneme') == 'sil') :
            ...
            if (match.group('cur_phone_position') == '1') :
                ...
                od['stress'] = int(get_stress_or_unstress(name_index))
...
def get_stress_or_unstress(name_index):
    if len(np.where(stress_list==name_index)[0]) != 0:
        global stress_count 
        stress_count += 1
        return 1
    elif len(np.where(unstress_list==name_index)[0]) != 0:
        global unstress_count 
        unstress_count += 1
        return 0
    else:
        global unclass_count
        unclass_count += 1
        print 'Cannot find name index : {}'.format(name_index)
        return 0
...
if __name__ == '__main__':
    ...
    outpath = '/work/w2/decha/Data/GPR_data/label/10_stress_unsupervised_labeling/utt/tsc/sd/'
    Utility.make_directory(outpath)

    stress_list = Utility.load_obj('/home/h1/decha/Dropbox/python_workspace/Inter_speech_2016/Gen_context_from_latent_variable/stress_name_list.npy')
    unstress_list = Utility.load_obj('/home/h1/decha/Dropbox/python_workspace/Inter_speech_2016/Gen_context_from_latent_variable/unstress_name_list.npy')
    ...
    # 1248 9493 30
    print stress_count, unstress_count, unclass_count
