...
def run_latent_data(base_path, trn_data, tst_data, outpath, name, fold):
    ...
    trndata = ANN_Executioner_Helper.get_latent_data(base_path, trn_data['name_index'], trn_data['stress'], use_input_sensitivity=True, normalize=False)
    tstdata = ANN_Executioner_Helper.get_latent_data(base_path, tst_data['name_index'], tst_data['stress'], use_input_sensitivity=True, normalize=False)

    # print 'No use input sensitivity'
    # trndata = ANN_Executioner_Helper.get_latent_data(base_path, trn_data['name_index'], trn_data['stress'], use_input_sensitivity=False, normalize=False)
    # tstdata = ANN_Executioner_Helper.get_latent_data(base_path, tst_data['name_index'], tst_data['stress'], use_input_sensitivity=False, normalize=False)
...
def run(fold_object_path, number_of_fold, outpath, feature, duration, delta_bool, delta2_bool, latent_data_path):
    ...
    for i in range(number_of_fold):
        ...
        trn_data = ANN_Executioner_Helper.get_data_from_obj(trn_obj, feature, duration, get_only_stress=None)
        tst_data = ANN_Executioner_Helper.get_data_from_obj(tst_obj, feature, duration, get_only_stress=None)

        trn_data_set = ANN_Executioner_Helper.get_ClassificationDataSet(trn_data['Y'], trn_data['stress'])
        tst_data_set = ANN_Executioner_Helper.get_ClassificationDataSet(tst_data['Y'], tst_data['stress'])
...
if __name__ == '__main__':
    ...
    out_base = '/work/w13/decha/Inter_speech_2016_workplace/Tonal_projection/09b_25lf0_fix_some_500_max_iters/'
    ...
    for i, j in itertools.product(range(len(vowel_type)), range(len(tones))):
        ...
        for d in deltas:
            ...
            for feature in features_type:
                run_add_and_non_duration(out_base, vowel, d, tone, '25lf0_c', [3,5,10])
