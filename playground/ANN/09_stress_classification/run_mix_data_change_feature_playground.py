...
def run(fold_object_path, number_of_fold, outpath, feature, duration, delta_bool, delta2_bool, latent_data_path):
    ...
    for i in range(number_of_fold):
        ...
        trn_data = ANN_Executioner_Helper.get_data_from_obj(trn_obj, feature, duration, get_only_stress=None, stress_unstress_must_equal=False, num_sampling=25, delta_bool=delta_bool, delta2_bool=delta2_bool)
        tst_data = ANN_Executioner_Helper.get_data_from_obj(tst_obj, feature, duration, get_only_stress=None, stress_unstress_must_equal=False, num_sampling=25, delta_bool=delta_bool, delta2_bool=delta2_bool)
        ...
        ANN_Executioner_Helper.run_nn_train_playground(trn_data_set, tst_data_set, outpath, 'Plain_data', i)
        print 'Latent data : '
    ...
    for i, j in itertools.product(range(len(vowel_type)), range(len(tones))):
        ...
        for d in deltas:
            ...
            for feature in features_type:
                ...
                run_add_and_non_duration(out_base, vowel, d, tone, '25lf0_50_playground', [3])
