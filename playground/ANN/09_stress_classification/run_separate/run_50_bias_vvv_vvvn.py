...
def run(fold_object_path, number_of_fold, outpath, feature, duration, delta_bool, delta2_bool, latent_data_path):
    ...
    for i in range(number_of_fold):
        ...
        trn_data = ANN_Executioner_Helper.get_data_from_obj(trn_obj, feature, duration, get_only_stress=None, stress_unstress_must_equal=False, delta_bool=delta_bool, delta2_bool=delta2_bool)
        tst_data = ANN_Executioner_Helper.get_data_from_obj(tst_obj, feature, duration, get_only_stress=None, stress_unstress_must_equal=False, delta_bool=delta_bool, delta2_bool=delta2_bool)
        ...
        print "Input and output dimensions: ", tst_data_set.indim, tst_data_set.outdim

        print 'Plain data : '
        # ANN_Executioner_Helper.run_nn(trn_data_set, tst_data_set, outpath, 'Plain_data', i)
        ANN_Executioner_Helper.run_nn_train_until_convergence(trn_data_set, tst_data_set, outpath, 'Plain_data', i)
        print 'Latent data : '
    ...
    for i, j in itertools.product(range(len(vowel_type)), range(len(tones))):
        ...
        for d in deltas:
            ...
            for feature in features_type:
                ...
                run_add_and_non_duration(out_base, vowel, d, tone, '25lf0_50e_iters_ann', [3,5,10])
