...
def run_latent_data(base_path, trn_data, tst_data, outpath, name, fold):
    ...
    trndata = ANN_Executioner_Helper.get_latent_data(base_path, trn_data['name_index'], trn_data['tone'], use_input_sensitivity=True, normalize=True)
    tstdata = ANN_Executioner_Helper.get_latent_data(base_path, tst_data['name_index'], tst_data['tone'], use_input_sensitivity=True, normalize=True)

    # ANN_Executioner_Helper.run_nn(trndata, tstdata, outpath, name, fold)
    ANN_Executioner_Helper.run_nn_train_tone_classification(trndata, tstdata, outpath, name, fold)
...
def run(fold_object_path, number_of_fold, outpath, feature, duration, delta_bool, delta2_bool, latent_data_path):
    ...
    for i in range(number_of_fold):
        ...
        trn_data = ANN_Executioner_Helper.get_data_from_obj(trn_obj, feature, duration)
        tst_data = ANN_Executioner_Helper.get_data_from_obj(tst_obj, feature, duration)

        trn_data_set = ANN_Executioner_Helper.get_ClassificationDataSet(trn_data['Y'], trn_data['tone'])
        tst_data_set = ANN_Executioner_Helper.get_ClassificationDataSet(tst_data['Y'], tst_data['tone'])
        ...
        ANN_Executioner_Helper.run_nn_train_tone_classification(trn_data_set, tst_data_set, outpath, 'Plain_data', i)
        print 'Latent data : '
...
def run_add_and_non_duration(out_base, vowel, d, tone, feature_name, num_dims):
        ...
        for n in num_dims:
            ...
            print 'Vowel : No_duration_{}, Delta : {}, Tone : {}'.format(vowel, d, tone)

            base_path = '{}/{}/input_dims_{}/delta-{}_delta-delta-{}/BGP_LVM_Tone_{}/'.format(out_base, 'No_duration_'+vowel, n, d[0], d[1] ,tone)
            syllable_object = '{}/{}/syllable_object_{}_fold/syllable_object_{}_3-fold_'.format(object_base, vowel, tone, tone)
            outpath = '{}/{}/input_dims_{}/delta-{}_delta-delta-{}/BGP_LVM_Tone_{}/feed_forword_ann/{}/'.format(out_base, 'No_duration_'+vowel, n, d[0], d[1], tone, feature_name)
...
if __name__ == '__main__':
    ...
    vowel_type = ['all_vowel_type', 'vvv', 'vvvn', 'vvvsg']
    tones = ['01234']
    ...
    out_base = '/work/w13/decha/Inter_speech_2016_workplace/Tonal_projection/10_tone_classification/'
    ...
    for i, j in itertools.product(range(len(vowel_type)), range(len(tones))):
        ...
        for d in deltas:
            ...
            for feature in features_type:
                run_add_and_non_duration(out_base, vowel, d, tone, '25lf0_fix_b', [3,5,10])
